...
const getActiveParam = (object, param) => {
  let activeParam = false
  for (const key in object) {
    if (object.hasOwnProperty(key) && key !== 'price' && key !== '__typename') {
      const element = object[key]
      element.map((item) => {
        if (item.slug === param) {
          activeParam = { key, ...item }
        }
      })
      if (activeParam) {
        return activeParam
...
const getFilterCreator = (query, activeParam) => {
  if (query[activeParam.key]) {
    if (Array.isArray(query[activeParam.key])) {
      return {
        ...query,
        [activeParam.key]: [...query[activeParam.key], activeParam.slug],
      }
    } else {
      return {
        ...query,
    ...
    return {
      ...query,
      [activeParam.key]: [activeParam.slug],
    }
  }
}
...
function CatalogKitchenPage({ kitchensPath }) {
  ...
  const pageSlug = `${kitchensStatic.filter.pageSlug}/${activeParam.slug}`
  const filter = useMemo(
    (s) => createFilter(getFilterCreator(router.query, activeParam)),
    [router.query]
  )
  const [previousFilter, setPreviousFilter] = useState(
    createFilter(getFilterCreator(router.query, activeParam))
  )
...
export async function getServerSideProps(ctx) {
  ...
  const activeParam = getActiveParam(data.kitchen_filter, query.param)
  await apolloClient.query({
  ...
  await apolloClient.query({
    ...
    variables: {
      ...
      filter: createFilter(getFilterCreator(query, activeParam)),
    },
  })
  await apolloClient.query({
