...
const getTags = (query) => {
  if (!query.tags) return query.tag
  if (Array.isArray(query.tags)) {
    return [...query.tags, query.tag]
  } else {
    return [query.tags, query.tag]
  }
}
...
function CatalogSinksPage({ sinksPath }) {
  ...
  const { tags, group, tag } = router.query
  const pageSlug = `${PATH.SINKS}/${group}/${tag}`
